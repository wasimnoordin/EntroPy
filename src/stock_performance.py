...
def calculate_cumulative_return(input_stock_prices, cumulative_dividend=None):
    ...
    if not isinstance(input_stock_prices, pd.DataFrame):
...
def calculate_daily_return_weighted(input_stock_prices, allocation):
    ...
    if not isinstance(allocation, (np.ndarray, pd.Series)):
...
def calculate_daily_return_logarithmic(input_stock_prices):
    ...
    calculate_log = lambda x: np.log(1 + x)
...
def calculate_historical_avg_return(stock_data, regular_trading_days=252):
    ...
    if not isinstance(stock_data, (pd.DataFrame, pd.Series)):
